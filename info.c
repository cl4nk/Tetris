...
void area_next_pieces_num_draw (GtkWidget*win, GdkGC *gc, int size_cell, int skin) {
	...
	for (int i = 0; i < NB_NEXT_PIECES; i++) {
		...
		gc_set_rgb_fg (gc, color);
		font_draw_text (win, gc, layout, FONT_TL,  (tab_x[i] + SIZE_PIECE + 0.5) * size_cell, (tab_y[i] + SIZE_PIECE + 0.5) * size_cell, "%d", i + 1);
		g_object_unref (G_OBJECT (layout));
	} 
...
void animatedText_draw (GtkWidget *win, GdkGC *gc, AnimatedText * aText, int size_cell, int skin) {
	...
	PangoLayout *layout = gtk_widget_create_pango_layout (win, NULL);
 	...
 	int color = colors_font [skin];
	gc_set_rgb_fg (gc, color);
	font_draw_text (win, gc, layout, FONT_TL, GRID_X + aText->x* size_cell + size_cell/2, GRID_Y + aText->y * size_cell + size_cell/2, "+%d", aText->int_to_print);
	g_object_unref (G_OBJECT (layout));
}
	...
	for (int i = 0; i < NB_PATTERN_COLS; i ++) {
		for (int j = 0; j < NB_PATTERN_ROWS; j++) {
			...
			if (piece->config.patterns[piece->current_rotation][i][j] == 1) {
				cell_draw (win, gc, TRUE, GRID_X + (i + x) * size_cell , GRID_Y + (j + y) * size_cell, size_cell, size_cell, piece->color, skin, img);
			}
		}
	}
}
...
int info_piece_can_be_added (Info * info) {
	...
	for (int i = 0; i < NB_PATTERN_COLS; i++)
		...
		{	
			...
			if (piece->config.patterns[piece->current_rotation][i][j] != 0 && info->grid[i + info->pos_x][j + info->pos_y] >= 0) return FALSE;
		}
	...
	for (int y = 0; y < SIZE_PIECE; y++) {
		...
		for (int x = 0; x < SIZE_PIECE; x ++) {
			if (piece->config.patterns[piece->current_rotation][x][y] != 0) {
				if (info->pos_x + x + - 1 < 0)
					return FALSE;
				else if (info->grid[info->pos_x + x - 1][info->pos_y + y ] >= 0)
					return FALSE;
				break;
			}
		}
		...
		for (int y = 3; y >= 0; y --) {
			...
			if (piece->config.patterns[piece->current_rotation][x][y] != 0) {
				if (info->pos_y + y + 1 >= info->height)
					return FALSE;
				else if (info->grid[info->pos_x + x][info->pos_y + y + 1] >= 0)
					return FALSE;
				break;
			}
		}
		...
		for (int y = 3; y >= 0; y --) {
			...
			if (piece->config.patterns[piece->current_rotation][x][y] != 0) {
				if (pos_y + y + 1 >= info->height)
					return FALSE;
				else if (info->grid[pos_x + x][pos_y + y + 1] >= 0)
					return FALSE;
				break;
			}
		}
	...
	if (!keys_are_equals(info->key_Down, info->key_Up)) {
		if (info->key_Down) {
			if (info_piece_can_go_down(info)) {
				...
				animatedText_setup(info->aText_down, 2*info->current_level, 500, info->pos_x + SIZE_PIECE / 2 - 1 , info->pos_y - 1);
			}
			else 
			{ 
				info_add_piece_to_grid (info);
...
void cell_draw (GdkWindow *win, GdkGC *gc , int filled, int x, int y, int width, int height, int color, int skin, GdkPixbuf * img) {
	...
	for (i = 0; i < info->width; i ++) {
		for (j = 0; j < info->height; j++) {
			int is_filled = info->grid[i][j] >= 0;
				if (is_filled) {
					cell_draw (win, gc, TRUE, GRID_X + i * size_cell, GRID_Y + j * size_cell, size_cell, size_cell, info->grid[i][j], info->skin, info->cell_img);
				}
...
void info_bonus_draw (GdkWindow *win, GdkGC *gc, Info* info, int size_cell) {
	...
	switch (info->bonus->type) {
		...
		case (CELL_BONUS) :
			...
			gdk_draw_rectangle(win, gc, FALSE, GRID_X + x * size_cell - 1, GRID_Y + y * size_cell - 1, size_cell + 2, size_cell + 2);
			return;
		case (BLOCK_BONUS) : 
			gdk_draw_rectangle(win, gc, FALSE, GRID_X + x * size_cell , GRID_Y + y * size_cell , size_cell * 2, size_cell * 2);
			gdk_draw_rectangle(win, gc, FALSE, GRID_X + x * size_cell - 1, GRID_Y + y * size_cell - 1, size_cell * 2 + 2, size_cell * 2 + 2);
			return;
		case (LINE_BONUS) :
			gdk_draw_rectangle(win, gc, FALSE, GRID_X  , GRID_Y + y * size_cell , size_cell * info->width, size_cell);
			gdk_draw_rectangle(win, gc, FALSE, GRID_X -1, GRID_Y + y * size_cell - 1, size_cell * info->width + 2, size_cell + 2);
			return;
		default : 
			return;
	}	
